...
namespace Entidades
{
    public class Numero
    {
        ...
        public string BinarioDecimal(string binario)
        {
            ...
            //Verifica que el string ingresado sea un binario y lo acumula. Caso contrario dejara vacio al acumulador
            for (i = 0; i < binario.Length && binario != "Valor inválido"; i++)
            {
                if (bin[i] != '1' && bin[i] != '0')
                {
                    if(bin[i] == '-' && contNegativo==0)
                    {
                        contNegativo++;
                        continue;
                    }
            ...
            //Si el acumulador es valido, osea poertenece a un binario hara la conversion requerida
            if(acumulador!="")
            {
